...
checkLM(final_dataset_lumbar, path="Data/ply/lumbar_ply/", pt.size = 2, suffix=".ply", render = "s", begin = 117, point = "s")
...
spheres3d(coords_lumbar[,,3], r = 0.002)

#checkLM(final_dataset_lumbar, path="", pt.size = 2, suffix=".ply", render="s", begin = 65) 
...
warp_specimen_lumbar <- coords_lumbar[,,131] #number displayed by findMeanSpec
warp_specimen_lumbar

#Import simplified mesh to create warp mesh on
mesh_3D_lumbar <- read.ply("Data/ply/refmesh_lumbar.ply") #make sure NO binary encoding (ASCII)

#Check range of mesh and coordinates to make sure it has same scale
range(mesh_3D_lumbar$vb[1:3,]) #if this is too big/small, scale in editor and re-import
...
sink("PCA_all_lumbar_components.txt")
...
PC1min_all_lumbar_points <- c(
  spheres3d(PC1min_all_lumbar[centrum_lumbar,], radius=.001, color = "red"),
  spheres3d(PC1min_all_lumbar[process_lumbar,], radius=.001, color = "blue"))

PC1max_all_lumbar_points <- c(
  spheres3d(PC1min_all_lumbar[centrum_lumbar,], radius=.001, color = "red"),
  spheres3d(PC1min_all_lumbar[process_lumbar,], radius=.001, color = "blue"))

PC2min_all_lumbar_points <- c(
  spheres3d(PC1min_all_lumbar[centrum_lumbar,], radius=.001, color = "red"),
  spheres3d(PC1min_all_lumbar[process_lumbar,], radius=.001, color = "blue"))

PC2max_all_lumbar_points <- c(
  spheres3d(PC1min_all_lumbar[centrum_lumbar,], radius=.001, color = "red"),
  spheres3d(PC1min_all_lumbar[process_lumbar,], radius=.001, color = "blue"))
...
PC1min_all_lumbar_mesh <- warpRefMesh(mesh = mesh_3D_lumbar, mesh.coord = warp_specimen_lumbar, 
...
##Create warp mesh, to use as reference for visualization of analyses
PC1max_all_lumbar_mesh <- warpRefMesh(mesh = mesh_3D_lumbar, mesh.coord = warp_specimen_lumbar, 
                                        ref = PC1max_all_lumbar, color = "gray", centered = FALSE)
##Save warped mesh as ply (Morpho)
mesh2ply(PC1max_all_lumbar_mesh, filename = "output/PC1max_all_lumbar")

##Create warp mesh, to use as reference for visualization of analyses
PC2min_all_lumbar_mesh <- warpRefMesh(mesh = mesh_3D_lumbar, mesh.coord = warp_specimen_lumbar, 
                                        ref = PC2min_all_lumbar, color = "gray", centered = FALSE)
##Save warped mesh as ply (Morpho)
mesh2ply(PC2min_all_lumbar_mesh, filename = "output/PC2min_all_lumbar")

##Create warp mesh, to use as reference for visualization of analyses
PC2max_all_lumbar_mesh <- warpRefMesh(mesh = mesh_3D_lumbar, mesh.coord = warp_specimen_lumbar, 
...
PCA_all_lumbar_ggplot <- ggplot(pcscores_all_lumbar, aes(x = Comp1, y = Comp2, label = Id, colour = age, fill = age))+
  ...
  scale_shape_manual(name = "Sex", labels = c( "F"   ,    "M"      , "unknown"), #copy from as.factor(sex)
                     values = shapes)+
  theme_bw()+
  xlab("PC 1 (49.15%)")+ #copy this from standard PCA plot (PCA_all_plot)
  ylab("PC 2 (20.44%)")+
...
PCA_all_lumbar_age_ggplot <- ggplot(pcscores_all_lumbar, aes(x = Comp1, y = Comp2, colour = age))+
...
PCA_all_lumbar_sex_ggplot <- ggplot(pcscores_all_lumbar[!pcscores_all_lumbar$sex%in% c("unknown"),], aes(x = Comp1, y = Comp2, colour = sex))+
...
#Get spec number
match("1961_6_14_45_1_d", dimnames(final_dataset_lumbar)[[3]])

#Import simplified ply
#Less faces, no holes or isolated triangles
refmesh_lumbar <- vcgImport("Data/ply/lumbar_ply/1961_6_14_45_1_d.ply")

#Plot on surface
fixed_LMs <- c(1:16)

shade3d(refmesh_lumbar, col = "white", alpha = 0.5, fastTransparency = T)
spheres3d(slidedlms_lumbar[fixed_LMs,,53], col =  "royalblue4", type = "s",
          radius = 3, aspect = T, main = "mean",axes = F, main = F, fov = 0)
spheres3d(slidedlms_lumbar[-fixed_LMs,,53], col =  "royalblue1", type = "s",
          radius = 2, aspect = T, main = "mean",axes = F, main = F, fov = 0)

rgl.snapshot(filename = "landmarks_lumbar_dorsal.png")
rgl.snapshot(filename = "landmarks_lumbar_lateral.png")
rgl.snapshot(filename = "landmarks_lumbar_posterior.png")
rgl.snapshot(filename = "landmarks_lumbar_anterior.png") 
